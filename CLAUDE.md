...
> Generated by the `generate_notifications()` Postgres function — run by the pg_cron job `daily-notifications` (6:00 daily) and via RPC on vendor saves. One notification per (user, type, reference); marking it read silences it permanently while the condition persists (re-notify only after 60 days). Past-due alerts exclude `cleared`/`disputed`/`void` invoices. In-app only — no email or SMS. The old `generate-notifications` edge function is retired.
...
- Generated by the `generate_notifications()` Postgres function (pg_cron daily + on vendor saves); dedupe is read-aware so cleared notifications do not reappear
