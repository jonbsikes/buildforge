...
export default function BuildForgeRedesign() {
  ...
  if (view.startsWith("mobile")) {
    return (
      <div className="min-h-screen bg-slate-200 flex flex-col items-center justify-center py-10" style={{ fontFamily: "'Inter', system-ui, -apple-system, sans-serif" }}>
        <div className="mb-6 text-center">
          <button onClick={() => setView("overview")} className="text-sm text-blue-600 font-medium hover:text-blue-700 flex items-center gap-1 mx-auto mb-2">
            <ArrowLeft size={14} /> Back to overview
          </button>
          <h2 className="text-lg font-bold text-slate-900">
            {view === "mobile-dash" ? "Mobile Dashboard" : view === "mobile-project" ? "Mobile Project Detail" : "Mobile AP"}
          </h2>
          <p className="text-sm text-slate-500">Shown in iPhone frame — scroll inside the device</p>
        </div>

        <PhoneFrame>
          {view === "mobile-dash" && (
            <MobileDashboard onProjectTap={(p) => { setSelectedProject(p); setView("mobile-project"); }} />
          )}
          {view === "mobile-project" && selectedProject && (
            <MobileProjectDetail project={selectedProject} onBack={() => setView("mobile-dash")} />
          )}
          {view === "mobile-ap" && <MobileAPView />}

          {/* Bottom Tab Bar inside phone */}
          <BottomTabBar
            active={view === "mobile-dash" ? "home" : view === "mobile-project" ? "projects" : "money"}
            onNavigate={(key) => {
              if (key === "home") setView("mobile-dash");
              if (key === "projects") setView("mobile-dash");
              if (key === "money") setView("mobile-ap");
            }}
            onFab={() => setShowFab(true)}
          />
        </PhoneFrame>

        {showFab && <QuickActionSheet onClose={() => setShowFab(false)} />}

        {/* Screen switcher */}
        <div className="flex gap-2 mt-6">
          {[
            { key: "mobile-dash", label: "Dashboard" },
            { key: "mobile-project", label: "Project Detail" },
            { key: "mobile-ap", label: "AP" },
          ].map(({ key, label }) => (
            <button key={key} onClick={() => { setView(key); if (key === "mobile-project") setSelectedProject(PROJECTS[0]); }}
              className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${
                view === key ? "bg-blue-600 text-white" : "bg-white text-slate-600 hover:bg-slate-100"
              }`}>
              {label}
            </button>
          ))}
  ...
  if (view.startsWith("desktop")) {
    return (
      <div className="h-screen flex bg-slate-50" style={{ fontFamily: "'Inter', system-ui, -apple-system, sans-serif" }}>
        {/* Navigation Rail */}
        <DesktopNav
          active={view === "desktop-dash" ? "home" : "invoices"}
          onNavigate={(key) => {
            if (key === "home") setView("desktop-dash");
            if (key === "invoices") setView("desktop-ap");
          }}
        />

        {/* Content */}
        {view === "desktop-dash" && <DesktopDashboard onProjectTap={() => {}} />}
        {view === "desktop-ap" && <DesktopAPTable />}

        {/* View switcher */}
        <div className="fixed bottom-6 right-6 flex gap-2 z-40">
          <button onClick={() => setView("overview")} className="px-4 py-2 bg-slate-900 text-white rounded-lg text-sm font-medium shadow-lg hover:bg-slate-800 flex items-center gap-2">
            <ArrowLeft size={14} /> Overview
          </button>
          <button onClick={() => setView(view === "desktop-dash" ? "desktop-ap" : "desktop-dash")}
            className="px-4 py-2 bg-blue-600 text-white rounded-lg text-sm font-medium shadow-lg hover:bg-blue-700">
            {view === "desktop-dash" ? "View AP Table" : "View Dashboard"}
          </button>
        </div>
      </div>
    );
