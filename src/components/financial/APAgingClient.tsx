        ...
        <>
                          ...
                          <tbody>
                            {bucketRows.map(row => (
                              <tr key={row.id} className="border-b border-gray-50 hover:bg-gray-50 transition-colors">
                                ...
                                <td className="px-5 py-2.5 text-gray-500">{fmtDate(row.invoice_date)}</td>
                                <td className="px-5 py-2.5 text-gray-500">{fmtDate(row.due_date)}</td>
                                <td className="px-5 py-2.5 text-right font-medium text-gray-800">{fmtFull(row.amount)}</td>
          ...
          <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
            <div className="px-5 py-3 border-b border-gray-100 bg-slate-700 flex items-center justify-between">
              ...
              {checksTotal > 0 && (
                <span className="text-sm font-bold text-white ml-4 shrink-0">{fmt(checksTotal)}</span>
              )}
            </div>
                  ...
                  <table className="w-full text-sm">
                    ...
                    <tbody>
                      {outstandingChecks.map((c) => (
                        <tr key={c.id} className={`border-b border-gray-50 hover:bg-gray-50 transition-colors ${c.days_outstanding > 30 ? "bg-amber-50/40" : ""}`}>
                          <td className="px-5 py-2.5 font-medium text-gray-800">{c.vendor_name}</td>
                          <td className="px-5 py-2.5 text-gray-600">{c.check_number ? `#${c.check_number}` : "—"}</td>
                          <td className="px-5 py-2.5 text-gray-500 text-xs">
                            {c.draw_date ? fmtDate(c.draw_date) : "—"}
                          </td>
                          <td className="px-5 py-2.5 text-gray-500">{fmtDate(c.payment_date)}</td>
                          <td className={`px-5 py-2.5 text-right font-medium ${c.days_outstanding > 30 ? "text-amber-600" : "text-gray-600"}`}>
                            {c.days_outstanding}d
                            {c.days_outstanding > 30 && <span className="ml-1 text-xs text-amber-500">⚠</span>}
                          </td>
                          <td className="px-5 py-2.5 text-right font-semibold text-gray-800">{fmtFull(c.amount)}</td>
                    ...
                    <tfoot>
                      <tr className="border-t-2 border-gray-200 bg-gray-50 font-bold">
                        ...
                        <td className="px-5 py-3 text-right text-gray-900">{fmt(checksTotal)}</td>
