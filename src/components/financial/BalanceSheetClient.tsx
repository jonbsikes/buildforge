...
/** Shape returned by the get_balance_sheet_data RPC. */
export type BalanceSheetRpcRow = {
  account_number: string;
  account_name: string;
  account_type: string;
...
/**
 * Pure transform from RPC rows (+ available vendor credits) to the report
 * shape. Runs on first render from server-provided props (Package 05 §B) and
 * again after client-side date-range refetches.
 */
function buildBalanceSheetData(rows: BalanceSheetRpcRow[], creditsAvailable: number): BalanceSheetData {
...
interface BalanceSheetClientProps {
  /** RPC rows fetched server-side for the default as-of date (today). */
  initialRows: BalanceSheetRpcRow[];
  /** Sum of available vendor credits as of the default date. */
  initialCredits: number;
  /** The as-of date the server data was fetched for. */
  initialAsOf: string;
}

export default function BalanceSheetClient({ initialRows, initialCredits, initialAsOf }: BalanceSheetClientProps) {
  const [asOf, setAsOf] = useState(initialAsOf);
  const [data, setData] = useState<BalanceSheetData>(() => buildBalanceSheetData(initialRows, initialCredits));
  ...
  // Client re-fetch only for date-range changes — the initial render is
  // server-fed. Same RPC as the server and the exports (no raw-line
  // aggregation in the client).
  const load = useCallback(async () => {
    ...
    setData(buildBalanceSheetData((rpcData ?? []) as BalanceSheetRpcRow[], creditsAvailable));
    setLoading(false);
  }, [asOf]);
