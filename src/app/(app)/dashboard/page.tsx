...
export default async function DashboardPage() {
  ...
  function getStageStripData(pid: string) {
    ...
    function buildStrip(trackStages: typeof stages): StageStripStage[] {
      ...
      if (nextUp) result.push({ name: nextUp.stage_name, status: "not_started", date: nextUp.planned_start_date ? fmtDate(nextUp.planned_start_date) : null });
      if (secondNext) result.push({ name: secondNext.stage_name, status: "not_started", date: secondNext.planned_start_date ? fmtDate(secondNext.planned_start_date) : null });
      return result;
    }
                  ...
                  {thisWeekStages.slice(0, 6).map((s) => {
                    ...
                    return (
                      <Link key={s.id} href={`/projects/${s.project_id}`} className="flex items-center gap-3 px-5 py-3 hover:bg-gray-50 transition-colors">
                        ...
                        <div className="flex-1 min-w-0"><p className="text-sm font-medium text-gray-900 truncate">{s.stage_name}</p><p className="text-xs text-gray-400">{projectNames[s.project_id] ?? "Unknown"} &middot; {isStart ? "Starts" : "Completes"} {fmtDate((isStart ? s.planned_start_date : s.planned_end_date) ?? today)}</p></div>
